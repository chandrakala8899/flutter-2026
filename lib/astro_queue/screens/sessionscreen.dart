...
//     }
//     return "007eJxTYIjQZGAzq3OYEfUiqEzzy3JPtzvhze9juxXW9a3c7R5aEqnAYJhobmmZnGZskGRgZpKYmJSUYmhmZGiRlmhgmmRiYGji6b8qsyGQkUFq2y0GRigE8QUZEotLivLjkzMS8/JSc+INjYwZGABnYiJx=";
//   }
...
class _SessionScreenState extends State<SessionScreen>
  ...
  void initState() {
    ...
    // ✅ Connect WebSocket for session events (extend, expiry notifications)
    ...
    _webSocketService.connect(
      ...
      onExpiryNotification: _handleExpiryNotification,
      onError: (error) => _log("WS Error: $error"),
    );
  ...
  // ✅ Handles all WebSocket session events
  void _handleSessionUpdate(Map<String, dynamic> data) {
    _log("🔥 SessionScreen WS Update: ${data['type']}");
    if (data['type'] == 'session_extended') {
      _handleSessionExtended(data);
    }
  }

  // ✅ When other side extends — update timer automatically
  void _handleSessionExtended(Map<String, dynamic> data) {
    ...
    if (newEndStr != null) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Session extended +$minutes mins ✓"),
            backgroundColor: Colors.green,
            behavior: SnackBarBehavior.floating,
            duration: const Duration(seconds: 2),
          ),
  ...
  void _handleExpiryNotification(Map<String, dynamic> data) {
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(data['message'] ?? 'Session expiring soon'),
  ...
  Future<void> _extendSession() async {
    ...
    try {
      ...
      if (result['success'] == true) {
        final newEndString = result['newScheduledEnd'] as String?;
        if (newEndString != null && newEndString.isNotEmpty) {
          final parsed = DateTime.tryParse(newEndString);
          if (parsed != null) _effectiveScheduledEnd = parsed;
        }

        final minutes = result['extendedMinutes'] ?? 15;

        setState(() => _isSessionEnded = false);
        _restartCountdownTimer();
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Row(children: [
              ...
              const SizedBox(width: 8),
              Text(result['message'] ?? 'Extended +$minutes mins ✓'),
            ]),
            backgroundColor: Colors.green,
            behavior: SnackBarBehavior.floating,
            duration: const Duration(seconds: 2),
  ...
  void _sendMessage() {
    final text = _messageController.text.trim();
    if (text.isEmpty || widget.session?.sessionId == null) return;
    final senderId = widget.isCustomer ? _uidCustomer : _uidPractitioner;
    _chatService.sendMessage(
      sessionId: widget.session!.sessionId!,
      senderId: senderId,
        ...
        body: _isStartingCall
            ...
            : Stack(
                children: [
                  ...
                  // ✅ Banner only for customer, only while in call
                  if (_isInCall && widget.session != null && widget.isCustomer)
                    Positioned(
                      top: 0,
                      left: 0,
                      right: 0,
